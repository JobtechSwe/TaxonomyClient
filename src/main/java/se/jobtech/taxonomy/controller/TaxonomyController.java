...
public class TaxonomyController {
    ...
    @GetMapping("/concept-from-id/{id}/{version}")
    @ResponseBody
    ...
    @ResponseBody
    private List<ChangesConceptEntity> searchSkill(@PathVariable String q, Long version) {
    ...
    @ResponseBody
    private List<ChangesConceptEntity> searchOccupationName(@PathVariable String q, Long version) {
    ...
    private List<ChangesConceptEntity> getReplacedConcepts(@PathVariable Long fromVersion, Long toVersion) {
    ...
    private void searchLoadTest(@PathVariable String q) {
        for (int i = 0; i < 50; i++) {
            ...
            for (char c : alphabet) {
                List<ChangesConceptEntity> t = searchConceptService.searchConcept(String.valueOf(c), "ssyk_level_4", null, null, 1L);
                System.out.println("Lista av ssyk_level_4 ConceptChanges Version1"+t);
                List<ChangesConceptEntity> s = searchConceptService.searchConcept(String.valueOf(c), "skill", null, null, 2L);
